...
export function route(input,{knowledge,memory,rules,references=[]}){
 ...
 if(/\b(?:fonte|fontes|link|links|referencia|referencias)\b/i.test(normalized)){
   ...
   return{response:last?.url?"Fonte: "+last.url:"Nenhuma fonte foi registrada nesta sessão.",source:"reference"}
 }
 if(/^(?:lembre|lembrar|guarde|guarda|memorize|memorizar|ensine|ensinar)\s+(?:que\s+)?/i.test(text)){
   const fact=normalize(text).replace(/^(?:lembre|lembrar|guarde|guarda|memorize|memorizar|ensine|ensinar)\s+(?:que\s+)?/,"");
 ...
 if(memoryQuestion){
   ...
   const names=facts.filter(f=>/\b(?:meu nome|me chamo|sou)\b/i.test(f));
 ...
 if(learned)return{response:chooseResponse(learned.item.responses),source:"learned"};
 const calc=extractCalculation(text);
 if(calc){const n=calculate(calc);if(n!==null)return{response:String(n),source:"calculator"}}
 const eliza=runEliza(text,rules);
 if(eliza)return eliza;
 const intent=detectIntent(text,knowledge.training);
 if(intent?.score>=.55)return{response:chooseResponse(intent.intent.responses),source:"chatterbot",intent:intent.intent.name};
 const concept=knowledge.findConcept(text);
 if(concept)return{response:concept.concept.response,source:"knowledge"};
 const entities=extractEntities(text);
 if(entities.length)return{response:"Entendi. Você mencionou "+entities.map(e=>e.value).join(", ")+". O que deseja fazer com isso?",source:"nlp"};
