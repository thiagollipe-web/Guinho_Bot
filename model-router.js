...
export function detectarComando(texto){
  ...
  const match=value.match(/^\\/(fast|qwen|nano)\\b/i);
  return match?match[1].toLowerCase():null;
}
export function removerComando(texto){return String(texto??"").trim().replace(/^\\/(fast|qwen|nano)\\b\\s*/i,"").trim();}
export function selecionarAutomaticamente(texto,{programacao=false,complexa=false}={}){
  const value=String(texto??"").toLowerCase();
  if(complexa||/\\b(arquitetura|refator|auditoria|analis[ae]r|diagn[oó]stico|projeto inteiro)\\b/.test(value))return "nano";
...
export function resolverModelo(texto,opcoes={}){
  ...
  if(command)return {key:command,model:obterModelo(command),prompt:removerComando(texto),source:"command"};
  const key=selecionarAutomaticamente(texto,opcoes);
  return {key,model:obterModelo(key),prompt:String(texto??"").trim(),source:"auto"};
}
export { MODELS };